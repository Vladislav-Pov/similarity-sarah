...
the local objectives of clients are not identical. The degree of homogeneity 
...
in directions consistent with the global function. Following this intuition, recent 
...
also known as the $\delta$-relatedness assumption or bounded Hessian dissimilarity. Exploiting this assumption helps distributed methods accelerate convergence, since in practice $\delta \ll L$ \cite{arjevani2015communication, kovalev2022optimal}.
...
In particular, if local objectives are constructed from i.i.d.\ samples, learning theory implies that $\delta$ decreases as $\mathcal{O}\left(\nicefrac{1}{\sqrt m}\right)$, where $m$ is the local dataset size \citep{hendrikx2020statistically}. When $\delta$ is small, each client's information becomes more relevant for longer periods of time, meaning that every worker can do more local steps. Remarkably, algorithms such as \texttt{FedAvg} and \texttt{FedProx} do not exploit this regime and their convergence rate does not improve as the homogeneity between clients grows, so the benefits of similarity remain unused. 

Early communication-efficient methods aimed to minimize number of communication rounds \citep{mcmahan2017communication, stich2018local, kovalev2022optimal}. This metric is appropriate when devices and communication channels are considered to be equivalent. However, it is not the case of real-world systems. The server cannot aggregate updates from all clients simultaneously, and even a single full synchronization round transmits $\mathcal{O}(n)$ vectors. Therefore, one should also consider the total amount of transmitted vectors, which counts cost per round and their number. This naturally motivates partial participation, where only a subset of clients operates with server at a time. \cite{mcmahan2017communication}. Under this measure, existing 
...
On the other hand, algorithms that eliminate full gradient calculation are developed for centralized problems, and do not consider local updates \cite{li2021zerosarah, medyakov2025variance}. If one transfers these methods to the distributed setting, where each gradient evaluation $\nabla f_i$ corresponds to communication with a client, the convergence rate will depend on smoothness constant $L$, rather than the $\delta$. Thus, making them insensitive to data homogeneity across clients.  This leaves open the question of whether one 
can exploit second-order similarity in distributed non-convex optimization and simultaneously avoid explicit full-gradient computation with 
theoretical guarantees.


\paragraph{Our Contribution.} \label{contribution}
We design a distributed method with local steps. Unlike existing similarity-based distributed methods, \textbf{our approach never computes or aggregates the full gradient at any step}. Second-order similarity is crucial in this setting, since it ensures that information obtained from sampled clients remains relevant to the global objective. To the best of our knowledge, this combination of complete no-full-gradient computation paradigm and second-order similarity is not covered by existing works.
...
\cite{kovalev2022optimal} proposed \texttt{Accelerated Extragradient}, whose complexity bound $\mathcal{O}\bigl(\sqrt{\delta/\mu} \, \log(1/ \varepsilon \bigr)$ 
on the number of rounds matches the lower bound of 
\cite{arjevani2015communication}. However, since each round in the 
full-participation setting requires all $n-1$ clients to transmit their 
gradients to the server, the number of \textit{exchanged vectors} grows as 
$\mathcal{O}\bigl(n\sqrt{\delta/\mu} \, \log(1/ \varepsilon\bigr)$. More recent works 
...
communicates per round. For example, \cite{khaled2022faster} proposed 
\texttt{SVRP} and \texttt{Catalyzed SVRP}, combining stochastic proximal 
point evaluations with variance reduction. \texttt{SVRP} attains 
$\mathcal{O}\bigl((n + \delta^{2}/\mu^{2})\log(1/\varepsilon)\bigr)$ 
communication complexity, and \texttt{Catalyzed SVRP} improves this to 
$\mathcal{O}\bigl((n + n^{3/4}\sqrt{\delta/\mu})\log(1/\varepsilon)\log(L/\mu)\bigr)$ 
via the Catalyst acceleration framework \cite{lin2015universal}. However, 
...
The update is identical to the proximal step in \texttt{Accelerated Extragradient}. However, computing $\nabla(f-f_1)$ requires gradients from all $n-1$ clients per round. We aim to replace $\nabla (f - f_1)$ by an estimator $v$. 
...
Because batches $B^{(s)}_1, B^{(s)}_2, \ldots, B^{(s)}_{\lceil n/b \rceil}$ are disjoint, and together cover all $n$ clients, $\tilde v^{(s)}_{\lceil n/b \rceil + 1}$ averages exactly one gradient from each client. We then set $v_0^{(s+1)} := \tilde v_{\lceil n/b \rceil + 1}^{(s)}$. The next two lemmas formalize how $\tilde v$ controls the variance. 
